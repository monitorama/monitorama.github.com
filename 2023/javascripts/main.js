...
// submit CFP form
if (eventCity == 'pdx') {
  $('.cfp .signup form').on('click', 'a', formHandler('https://judy-pdx-2023.herokuapp.com/abstracts/new'));
}
    ...
    $('.schedule span.speaker:contains(' + speakers[i].name + ')').parent().find('h5')
      ...
      .on('click', function (e) {
        ...
        if (results[0]) {
          ...
          $("#speakerModal #speakerAbstract").html(speaker.abstract || "Coming Soon");
...
for (var i in mySponsors) {
  if (mySponsors[i].name.length && mySponsors[i].enabled && mySponsors[i].url.length && mySponsors[i].bio.length) {
    ...
    $('div.marquee').append('<a href="' + mySponsors[i].url + '" target="_blank">' + '<img src="images/logo_' + mySponsors[i].name + '.png" /></a>');
  }
}
